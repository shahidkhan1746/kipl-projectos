...
export class PdfService {
  ...
  async generateMonthlyAttendanceReport(data: { year: number; month: number; records: any[]; employees: any[]; project?: any }): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      for (const rec of (data.records || [])) {
        if (!employeeRecords.has(rec.employeeId)) employeeRecords.set(rec.employeeId, [])
