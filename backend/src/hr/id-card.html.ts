// Employee ID card as a self-contained HTML document. Viewable/printable in the
// browser, or converted to PDF by Gotenberg. Two styles: 'template' (KIPL
// branded, two-sided) and 'minimal'. Card size 54 × 86 mm (lanyard).
...
function frontTemplate(emp: any): string {
...
function backTemplate(emp: any): string {
  const name = `${emp.firstName ?? ''} ${emp.lastName ?? ''}`.trim()
