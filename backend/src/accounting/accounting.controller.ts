...
export class AccountingController {
  ...
  // ── Reads (role-gated to financial / engineering personnel) ───────────────
  @Get('dashboard')
  @UseGuards(RolesGuard)
  @Roles(...FIN, UserRole.ENGINEER)
  dashboard(@Query('projectId') pid: string) {
  ...
  expenses(@Query() q: any) {
    return this.svc.listExpenses({ projectId: q.projectId, vendorId: q.vendorId, category: q.category, status: q.status, fromDate: q.fromDate, toDate: q.toDate })
  }
  ...
  transactions(@Query() q: any) {
    return this.svc.listTransactions({ projectId: q.projectId, vendorId: q.vendorId, fromDate: q.fromDate, toDate: q.toDate, type: q.type })
  }
  ...
  @Roles(...FIN)
  tds(@Query() q: any) {
    return this.svc.listTds({ projectId: q.projectId, quarter: q.quarter, fy: q.fy, status: q.status })
  }
  ...
  upload(@UploadedFile() file: any) {
    return this.storage.upload(file, 'expenses')
  }
  ...
  createVendor(@Body() body: any) {
    return this.svc.createVendor(body)
  }
  ...
  updateVendor(@Param('id') id: string, @Body() body: any) {
    return this.svc.updateVendor(id, body)
  }
  ...
  createExpense(@Body() body: any, @Request() req: any) {
    return this.svc.createExpense(body, req.user?.id)
  }
  ...
  updateExpense(@Param('id') id: string, @Body() body: any) {
    return this.svc.updateExpense(id, body)
  }
  ...
  approveExpense(@Param('id') id: string, @Request() req: any) {
    return this.svc.approveExpense(id, req.user?.id)
  }

  @Patch('expenses/:id/pay')
  @UseGuards(RolesGuard)
  @Roles(...FIN)
  payExpense(@Param('id') id: string, @Body() body: any) {
    return this.svc.markExpensePaid(id, body)
  }

  @Patch('expenses/:id/itc')
  @UseGuards(RolesGuard)
  @Roles(...FIN)
  setItc(@Param('id') id: string, @Body() body: any) {
    return this.svc.setItcClaimed(id, !!body.claimed)
  }
  ...
  @Roles(...FIN)
  @HttpCode(HttpStatus.CREATED)
  addTxn(@Body() body: any) {
    return this.svc.addTransaction(body)
  }

  @Patch('tds/:id/deposit')
  @UseGuards(RolesGuard)
  @Roles(...FIN)
  depositTds(@Param('id') id: string, @Body() body: any) {
    return this.svc.depositTds(id, body)
  }
