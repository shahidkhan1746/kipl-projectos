...
let WbsService = class WbsService {
    ...
    async seed(projectId) {
        ...
        if (existing > 0)
            return { seeded: 0 };
        const tasks = SEED_TASKS.map(t => this.repo.create({ ...t, projectId, status: wbs_task_entity_1.TaskStatus.NOT_STARTED, progressPct: 0 }));
    ...
    async dashboard(projectId) {
        ...
        const delayed = nonMilestones.filter(t => t.status === wbs_task_entity_1.TaskStatus.DELAYED || t.delayDays > 0).length;
        ...
        const passedMs = milestones.filter(t => {
            return t.status === wbs_task_entity_1.TaskStatus.COMPLETED || new Date(t.plannedEnd) < new Date();
        });
        const contractEnd = new Date('2028-03-27');
        const today = new Date();
        const daysRemaining = Math.round((contractEnd.getTime() - today.getTime()) / 86400000);
        const contractPct = Math.min(100, Math.max(0, (today.getTime() - new Date('2025-09-27').getTime()) /
            (contractEnd.getTime() - new Date('2025-09-27').getTime()) * 100)).toFixed(1);
        return {
