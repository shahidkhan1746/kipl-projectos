...
const FRONT = {
    photo: { top: 27.5, left: 50, w: 31 },
    values: { left: 44, right: 5, size: 2.6,
...
function templateCard(emp) {
    const name = `${emp.firstName ?? ''} ${emp.lastName ?? ''}`.trim();
    const V = FRONT.values;
    const val = (top, text) => `<div class="v" style="top:${top}%; left:${V.left}%; right:${V.right}%; font-size:${V.size}mm">${esc(text)}</div>`;
