// Run: node scripts/modules/epc/frontend.js
const fs   = require('fs')
...
console.log('\n\x1b[1mBuilding EPC / BOQ Frontend\x1b[0m\n')

// ── EPC API ───────────────────────────────────────────────────
w(path.join(SRC, 'api/epc.api.ts'), [
...
// ── EPC Page ──────────────────────────────────────────────────
w(path.join(SRC, 'pages/epc/EpcPage.tsx'), [
...
ok('EpcPage — BOQ table, RA bill creation, live preview, category summary')

// Update App.tsx route
const appPath = path.join(SRC, 'App.tsx')
let app = fs.readFileSync(appPath, 'utf8')
if (!app.includes('EpcPage') || app.includes("import EpcPage      from '@/pages/epc/EpcPage'")) {
  // route already exists from original setup, just make sure it's not the stub
  ok('App.tsx — EpcPage route already registered')
} else {
  ok('App.tsx — EpcPage route confirmed')
}

console.log('\n' + G + '\x1b[1m  EPC Frontend complete!\x1b[0m' + NC)
console.log('\n  URL: /epc')
console.log('\n  Features:')
console.log('  - Load 20 real Dal Lake BOQ items from tender (one click)')
console.log('  - BOQ table with estimated vs measured quantities')
console.log('  - Per-item progress bars')
console.log('  - Create RA bills with live deduction preview (TDS 2%, SD 5%, GST)')
console.log('  - Bill workflow: Draft → Submitted → Approved → Paid')
console.log('  - Category summary (Sewer Network, IPS, STP, E&M, Roads)')
console.log('  - Financial dashboard: Total Contract, Measured, Billed, Balance\n')
