// Run: node scripts/modules/hr/timesheet-frontend.js
const fs   = require('fs')
...
console.log('\n\x1b[1mBuilding Timesheet Frontend\x1b[0m\n')

// ── ACTIVITY CATEGORIES (matching real timesheet) ─────────────
const CATEGORIES = [
  'Site Supervision',
  'Measurement & Survey',
  'Quality Check / QA',
...
w(path.join(SRC, 'pages/hr/TimesheetPage.tsx'), [
  "import { useState } from 'react'",
  "import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query'",
  "import { ClipboardText, Plus, Trash, CheckCircle, XCircle, Clock, FileText } from '@phosphor-icons/react'",
  ...
  "",
  "const C = { card:'#fff', border:'#e2e8f0', bg:'#f0f2f5', text1:'#0f172a', text2:'#475569', text3:'#94a3b8', blue:'#2563eb', green:'#059669', amber:'#d97706', red:'#dc2626', navy:'#1a2540' }",
  "",
  ...
  "    queryFn: () => hrApi.timesheets({ month: viewMonth + 1, year: viewYear, projectId: activeProjectId, employeeId: filterEmp || undefined }).then(r => r.data),",
  "  })",
  "",
  ...
  "      ...form,",
  "    }),",
  "    onSuccess: () => { qc.invalidateQueries({ queryKey: ['timesheets'] }); setShowSubmit(false) },",
  "  })",
  "",
  ...
  "    mutationFn: ({ id, reason }: any) => hrApi.rejectTimesheet(id, reason),",
  "    onSuccess: () => { qc.invalidateQueries({ queryKey: ['timesheets'] }); setViewTs(null) },",
  "  })",
  "",
  ...
  "  const pending   = (timesheets ?? []).filter((t: any) => t.status === 'submitted').length",
  "  const approved  = (timesheets ?? []).filter((t: any) => t.status === 'approved').length",
  "",
  "  return (",
  ...
  "        </div>",
  "        <Button variant='primary' size='md' icon={<Plus size={15}/>} onClick={()=>setShowSubmit(true)}>Submit Today's Log</Button>",
  "      </div>",
  "",
  "      {/* Stats + filters */}",
  "      <div style={{ display:'flex', alignItems:'center', gap:12 }}>",
  "        <div style={{ display:'flex', gap:3, background:C.card, border:'1.5px solid '+C.border, borderRadius:8, padding:3 }}>",
  "          <button onClick={()=>setViewMonth(m=>(m-1+12)%12)} style={{ padding:'6px 10px', background:'none', border:'none', cursor:'pointer', color:C.text3, fontSize:14 }}>‹</button>",
  "          <span style={{ padding:'6px 12px', fontSize:13, fontWeight:700, color:C.text1 }}>{monthName} {viewYear}</span>",
  "          <button onClick={()=>setViewMonth(m=>(m+1)%12)} style={{ padding:'6px 10px', background:'none', border:'none', cursor:'pointer', color:C.text3, fontSize:14 }}>›</button>",
  "        </div>",
  ...
  "          {[{label:'Total Logs', value:(timesheets??[]).length, color:C.blue},{label:'Pending Approval', value:pending, color:C.amber},{label:'Approved', value:approved, color:C.green}].map(s=>(",
  ...
  "          <h2 style={{ fontSize:14, fontWeight:700, color:C.text1, margin:0 }}>",
  "            {monthName} {viewYear} — {(timesheets??[]).length} entries",
  "          </h2>",
  "        </div>",
  ...
  "                          <span style={{ width:6, height:6, borderRadius:'50%', background:C.blue, flexShrink:0, display:'inline-block' }} />",
  ...
  "                        <button onClick={()=>setViewTs(ts)} style={{ padding:'5px 10px', background:'none', border:'1.5px solid '+C.border, borderRadius:6, fontSize:11, color:C.text2, cursor:'pointer' }}>View</button>",
  "                        {ts.status==='submitted' && (",
  "                          <button onClick={()=>approveM.mutate(ts.id)} style={{ padding:'5px 10px', background:'#ecfdf5', border:'1.5px solid #a7f3d0', borderRadius:6, fontSize:11, color:'#047857', cursor:'pointer', fontWeight:600 }}>✓ Approve</button>",
  "                        )}",
  "                      </div>",
  "                    </td>",
  "                  </tr>",
  ...
  "          <Button variant='primary' loading={submitM.isPending} onClick={()=>submitM.mutate()} disabled={form.activities.every((a:any)=>!a.activity)}>",
  "            Submit Log",
  "          </Button>",
  "        </>}>",
  ...
  "              <button onClick={addActivity} style={{ fontSize:12, color:'#2563eb', background:'none', border:'none', cursor:'pointer', fontWeight:600, display:'flex', alignItems:'center', gap:4 }}>",
  "                <span style={{ fontSize:16 }}>+</span> Add activity",
  "              </button>",
  "            </div>",
  "            <div style={{ border:'1.5px solid #e2e8f0', borderRadius:10, overflow:'hidden' }}>",
  "              {form.activities.map((a: any, i: number) => (",
  "                <div key={i} style={{ display:'grid', gridTemplateColumns:'70px 1fr 140px 100px 32px', gap:8, padding:'10px 12px', borderBottom:i<form.activities.length-1?'1px solid #f1f5f9':'none', alignItems:'center', background:i%2===0?'#fff':'#fafafa' }}>",
  ...
  "                    <button onClick={()=>removeActivity(i)} style={{ background:'none', border:'none', cursor:'pointer', color:'#94a3b8', fontSize:16, lineHeight:1 }}>✕</button>",
  "                  )}",
  "                </div>",
  "              ))}",
  "            </div>",
  "            <p style={{ fontSize:11, color:'#94a3b8', margin:'6px 0 0' }}>Add one row per task. Time is optional. Category helps in reporting.</p>",
  "          </div>",
  "",
  "          {/* Summary fields */}",
  "          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>",
  ...
  "      {viewTs && (",
  "        <Modal open={!!viewTs} onClose={()=>setViewTs(null)} title={`Daily Log — ${new Date(viewTs.date).toLocaleDateString('en-IN',{day:'numeric',month:'long',year:'numeric'})}`} width={640}",
  "          footer={<>",
  "            {viewTs.status==='submitted' && (",
  "              <><Button variant='danger' size='sm' onClick={()=>{ const r=prompt('Reason for rejection:'); if(r) rejectM.mutate({id:viewTs.id,reason:r}) }}>Reject</Button>",
  "              <Button variant='success' size='sm' icon={<CheckCircle size={13}/>} loading={approveM.isPending} onClick={()=>{ approveM.mutate(viewTs.id); setViewTs(null) }}>Approve</Button></> )",
  "            }",
  ...
  "              <div>",
  "                <p style={{ fontSize:14, fontWeight:700, color:'#0f172a', margin:0 }}>{empMap[viewTs.employeeId]?`${empMap[viewTs.employeeId].firstName} ${empMap[viewTs.employeeId].lastName??''}`:'Employee'}</p>",
  "                <p style={{ fontSize:11, color:'#94a3b8', margin:'2px 0 0', fontFamily:'monospace' }}>{empMap[viewTs.employeeId]?.empCode??''}</p>",
  "              </div>",
  "              <div style={{ display:'flex', gap:10, alignItems:'center' }}>",
  "                <span style={{ fontSize:12, color:'#475569', textTransform:'capitalize', fontWeight:600 }}>{viewTs.attendanceStatus?.replace(/_/g,' ')}</span>",
  ...
  "            {/* Activities */}",
  "            <div>",
  "              <h3 style={{ fontSize:13, fontWeight:700, color:'#0f172a', margin:'0 0 10px' }}>Activities</h3>",
  "              <table style={{ width:'100%', borderCollapse:'collapse' }}>",
  "                <thead>",
  ...
  "                      <td style={{ padding:'9px 12px', fontSize:12, color:'#475569', whiteSpace:'nowrap' }}>{a.time||'—'}</td>",
...
if (!app.includes('TimesheetPage')) {
  ...
  ok('App.tsx — /hr/timesheets route added')
}

// Add to Sidebar
const sidebarPath = path.join(SRC, 'components/layout/Sidebar.tsx')
let sidebar = fs.readFileSync(sidebarPath, 'utf8')
if (!sidebar.includes('timesheets')) {
  sidebar = sidebar.replace(
    "{ label:'Attendance',",
    "{ label:'Timesheets',  path:'/hr/timesheets',      icon:ClipboardText, end:true  },\n    { label:'Attendance',"
  )
  sidebar = sidebar.replace(
    "import {\n  SquaresFour",
    "import {\n  SquaresFour, ClipboardText,"
  ).replace(
    'import {\n  SquaresFour, ClipboardText,',
    'import {\n  SquaresFour,'
  )
  // simpler approach - just add ClipboardText to existing import
  sidebar = sidebar.replace(
    "Buildings, SignOut,",
    "Buildings, SignOut, ClipboardText,"
  )
  fs.writeFileSync(sidebarPath, sidebar)
  ok('Sidebar — Timesheets link added under HR')
}

console.log('\n' + G + '\x1b[1m  Timesheet Frontend complete!\x1b[0m' + NC)
console.log('\n  URL: /hr/timesheets')
console.log('  Features:')
console.log('  - Submit daily activity log with multiple tasks')
console.log('  - Time, Activity description, Category, Location per row')
console.log('  - Work done summary, Issues faced, Next day plan')
console.log('  - PM can view, approve, or reject')
console.log('  - Filter by employee and month\n')
