// Run from project root: node scripts/modules/diary/build.js
const fs   = require('fs')
const path = require('path')

const DIARY = path.join('backend', 'src', 'diary')
...
fs.mkdirSync(path.join(FSRC, 'pages', 'diary'), { recursive: true })
fs.mkdirSync(path.join(FSRC, 'api'), { recursive: true })

console.log('\n\x1b[1mBuilding Site Daily Diary Module\x1b[0m\n')

// ── Entity (NO createdAt — BaseEntity provides it) ────────────
fs.writeFileSync(path.join(DIARY, 'diary.entity.ts'), `import { Entity, Column } from 'typeorm'
...
`)
ok('diary.entity.ts — no createdAt conflict')

// ── Service ───────────────────────────────────────────────────
fs.writeFileSync(path.join(DIARY, 'diary.service.ts'), `import { Injectable, NotFoundException, ConflictException } from '@nestjs/common'
...
export class DiaryService {
  ...
  async create(data: any): Promise<SiteDiary> {
    ...
    return this.repo.save(this.repo.create({ ...data, labourTotal: total }))
  }
...
// ── Controller ────────────────────────────────────────────────
fs.writeFileSync(path.join(DIARY, 'diary.controller.ts'), `import { Controller, Get, Post, Patch, Param, Body, Query, UseGuards, Request, HttpCode, HttpStatus } from '@nestjs/common'
...
`)
ok('diary.controller.ts')

// ── Module ────────────────────────────────────────────────────
fs.writeFileSync(path.join(DIARY, 'diary.module.ts'), `import { Module } from '@nestjs/common'
...
if (!app.includes("from './diary/diary.module'")) {
  app = app.replace("import { QaModule }", "import { DiaryModule } from './diary/diary.module'\nimport { QaModule }")
  app = app.replace('QaModule,', 'QaModule,\n    DiaryModule,')
  fs.writeFileSync(appPath, app)
  ok('DiaryModule registered in app.module.ts')
} else { ok('Already registered') }

// ── Frontend API ──────────────────────────────────────────────
fs.writeFileSync(path.join(FSRC, 'api', 'diary.api.ts'), `import api from './client'
...
// ── Frontend Page ─────────────────────────────────────────────
fs.writeFileSync(path.join(FSRC, 'pages', 'diary', 'DiaryPage.tsx'), `import { useState } from 'react'
...
`)
ok('DiaryPage.tsx — 4-step wizard (weather, labour, work, notes)')

// Add route to App.tsx
const appTsxPath = path.join(FSRC, 'App.tsx')
let appTsx = fs.readFileSync(appTsxPath, 'utf8')
if (!appTsx.includes('DiaryPage')) {
  ...
  ok('App.tsx — /diary route added')
}

// Add to Sidebar
const sidebarPath = path.join(FSRC, 'components', 'layout', 'Sidebar.tsx')
let sidebar = fs.readFileSync(sidebarPath, 'utf8')
if (!sidebar.includes("path:'/diary'")) {
  sidebar = sidebar.replace(
    "{ label:'Quality (QA)'",
    "{ label:'Site Diary', path:'/diary', icon:BookOpen, end:true },\n    { label:'Quality (QA)'"
  )
  sidebar = sidebar.replace('Buildings, SignOut, CheckSquare,', 'Buildings, SignOut, CheckSquare, BookOpen,')
  fs.writeFileSync(sidebarPath, sidebar)
  ok('Sidebar — Site Diary link added')
}

console.log('\n\x1b[32m\x1b[1m  Site Daily Diary complete!\x1b[0m' + NC)
console.log('\n  URL: /diary')
console.log('\n  Features:')
console.log('  - 4-step entry wizard: Weather → Labour → Work Done → Notes')
console.log('  - Weather: morning/afternoon, temp, rainfall, work stoppage')
console.log('  - Labour: skilled/unskilled/supervisory with auto-total')
console.log('  - Equipment: type, count, hours per machine')
console.log('  - Work done: zone-wise (all 13 IPS + MPS + STP + Rising Main)')
console.log('  - Materials received, site visitors log')
console.log('  - EOT claim flagging with reason — builds evidence for UEED')
console.log('  - Rainy days counter and total hours lost shown on dashboard')
console.log('  - Draft → Submit → Approve workflow\n')
