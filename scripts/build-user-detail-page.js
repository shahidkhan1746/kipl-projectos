...
// ── 1. Create UserDetailPage ──────────────────────────────────────────────────
const detailContent = `import { useState } from 'react'
...
const detailPath = path.join(ROOT, 'frontend', 'src', 'pages', 'settings', 'UserDetailPage.tsx')
fs.writeFileSync(detailPath, detailContent, 'utf8')
console.log('✅  UserDetailPage.tsx created')

// ── 2. Add route to App.tsx ───────────────────────────────────────────────────
const appPath = path.join(ROOT, 'frontend', 'src', 'App.tsx')
let app = fs.readFileSync(appPath, 'utf8')

if (!app.includes('UserDetailPage')) {
  app = app.replace(
  ...
  console.log('✅  Route /settings/users/:id added to App.tsx')
}

// ── 3. Make user rows clickable in SystemSettingsPage ─────────────────────────
const settingsPath = path.join(ROOT, 'frontend', 'src', 'pages', 'settings', 'SystemSettingsPage.tsx')
let settings = fs.readFileSync(settingsPath, 'utf8')

// Add useNavigate import
if (!settings.includes('useNavigate')) {
...
// Add nav inside UserManagement
if (!settings.includes('const nav = useNavigate')) {
  settings = settings.replace(
    `  const qc = useQueryClient()`,
    `  const nav = useNavigate()\n  const qc = useQueryClient()`
  )
...
// Make name cell clickable — navigate to user detail page
settings = settings.replace(
  ...
  `              <td style={{ padding:'13px 18px', borderBottom:'1px solid #f1f5f9', cursor:'pointer' }}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  ...
                  <span style={{ fontSize:13, fontWeight:600, color:C.blue, textDecoration:'underline' }}>{u.name}</span>
                </div>
              </td>`
)

fs.writeFileSync(settingsPath, settings, 'utf8')
console.log('✅  User names are now clickable links → /settings/users/:id')

console.log('\n🏁  User Detail Page complete')
console.log('   Click any user name → full profile page')
console.log('   Edit name/email/role/phone/department')
console.log('   Reset password inline')
